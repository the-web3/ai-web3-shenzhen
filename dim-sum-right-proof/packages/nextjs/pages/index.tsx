...
const ETHSpace: NextPage = () => {
  ...
  const [tokenizationProtocol, setTokenizationProtocol] = useState("");
  const [isCreatingDimSum, setIsCreatingDimSum] = useState(false);
  const [dimSumResponse, setDimSumResponse] = useState<{ success: boolean; data?: any; error?: string } | null>(null);
  ...
  // Handle DimSum API dataset creation
  const handleCreateDimSum = async () => {
    setIsCreatingDimSum(true);
    setDimSumResponse(null);

    try {
      const response = await fetch("https://backend.rightproof.app.aidimsum.com/dataset_licences", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          name: datasetName,
          context: `**Name**: ${datasetName}\n\n**Description**: ${datasetDescription}\n\n**Hash**: ${datasetHash}\n\n**License**: https://bodhi.wtf/${selectedLicense.bodhi_id}\n\n**Tokenized**: ${isTokenized ? "true" : "false"}${isTokenized && tokenizationProtocol ? `\n\n**Tokenization Protocol**:\n${tokenizationProtocol}` : ""}\n\n**Details**: ${datasetLink}`
        }),
      });
      ...
      if (response.ok) {
        setDimSumResponse({ success: true, data: data.data });
      } else {
        setDimSumResponse({ success: false, error: data.error || "Failed to create dataset" });
      }
    } catch (error) {
      console.error("Error creating dataset on DimSum:", error);
      setDimSumResponse({ success: false, error: "Network error. Please try again." });
    } finally {
      setIsCreatingDimSum(false);
    }
  };
...
`;
  ...
  return (
    <div className="grid lg:grid-cols-1 flex-grow">
      <div className="flex flex-col justify-center items-center bg-[url('/assets/gradient-bg.png')] bg-[length:100%_100%] py-10 px-5 sm:px-0 lg:py-auto max-w-[100vw] ">
        ...
        <div className="hero-content text-center">
          <div className="max-w-screen-xl">
            ...
            <h2 className="text-2xl font-bold mb-6"> 😎 Licence & Tokenize ur Dataset Step by Step! 😎</h2>
            {/* Step 1: Generate Data Hash */}
            <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
              <h3 className="text-lg font-semibold mb-2">Step 1: Generate Data Hash（可选）</h3>
              <h3 className="text-lg font-semibold mb-2">步骤 1:生成数据指纹（可选）</h3>
            ...
            <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
              ...
              <div className="form-control w-full max-w-2xl mx-auto space-y-4">
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-semibold">Dataset Hash / 数据集哈希值（可选）</span>
                    <span className="label-text-alt text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                ...
                <div className="form-control">
                  ...
                  {isTokenized && (
                    <button
                      type="button"
                      className="btn btn-secondary mt-2 text-dark"
                      onClick={() => setShowProtocolModal(true)}
                    >
                      📝 Input the Data Tokenization Protocol / 填写数据代币化协议，通常是描述未来数据利益如何分配（可选）
                    </button>
                  )}
                </div>
                ...
                <div className="mt-6 p-4 bg-white dark:bg-gray-800 rounded-lg border-2 border-primary">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {isTokenized && (
                      <p>
                        <strong>Data Tokenization Protocol:</strong>{" "}
                        {tokenizationProtocol ? (
                          <span className="block mt-1 text-gray-600 dark:text-gray-300 whitespace-pre-wrap text-xs">
                            {tokenizationProtocol.slice(0, 200)}
                            {tokenizationProtocol.length > 200 ? "..." : ""}
                          </span>
                        ) : (
                          <span className="text-gray-400">未填写</span>
                        )}
                ...
                <button
                  ...
                  disabled={!datasetName || !datasetDescription || isCreatingDimSum}
                  onClick={handleCreateDimSum}
                >
                  {isCreatingDimSum ? (
                    <>
                    ...
                    "🚀 Create Dataset Info on DimSum System"
                  )}
                </button>
                    ...
                    {dimSumResponse.success ? (
                      <div>
                        ...
                        <div className="text-sm space-y-1">
                          <p>
                            <strong>ID:</strong> {dimSumResponse.data?.id}
                          </p>
                          <p className="flex items-center justify-center gap-2 flex-wrap">
                            <strong>Unique ID:</strong>{" "}
                            <span className="font-mono text-xs break-all">{dimSumResponse.data?.unique_id}</span>
                            <button
                              className="btn btn-xs btn-ghost"
                              onClick={async () => {
                                try {
                                  await navigator.clipboard.writeText(dimSumResponse.data?.unique_id || "");
                                  setUniqueIdCopied(true);
                                  setTimeout(() => setUniqueIdCopied(false), 2000);
                                } catch (err) {
                                  console.error("Failed to copy:", err);
                                }
                              }}
                          ...
                          <p>
                            ...
                            {new Date(dimSumResponse.data?.created_at).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div>
                        ...
                        <p className="text-sm">{dimSumResponse.error}</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
              ...
              <div className="form-control">
                <label className="label">
                  <span className="label-text font-semibold">Bodhi ID or Unique ID in DimSum System</span>
                </label>
                <input
                  type="text"
                  placeholder="e.g., 15545 or 7ebce10b-e369-4bf9-9ad1-7db978b1f128"
                  className="input input-bordered w-full"
                  value={bodhiId}
                  onChange={e => setBodhiId(e.target.value)}
                />
                <label className="label">
                  <span className="label-text-alt">请填写「步骤 3 中生成的 Bodhi ID 」或「步骤 3 中生成的 Unique ID」</span>
                </label>
              </div>
              ...
              <div className="form-control w-full max-w-2xl mx-auto">
                <button
                  ...
                  disabled={!datasetName || !datasetDescription || isCreating}
      ...
      {/* Tokenization Protocol Modal */}
      {showProtocolModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold">📝 Data Tokenization Protocol / 数据代币化协议</h3>
                <button
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Please describe how future data benefits should be distributed among token holders.
                <br />
                请描述未来数据利益应如何在代币持有者之间分配。
              </p>
              <textarea
                className="textarea textarea-bordered w-full h-64 font-mono text-sm rounded-lg"
                placeholder={`# Data Tokenization Protocol

## Profit Distribution
- Token holders receive proportional share of all revenue
- 70% distributed to holders
- 20% to original creator
- 10% for maintenance

## Governance
- Major decisions require 51% token holder approval
- Token holders can vote on licensing changes

## Usage Rights
- Each token grants access rights to the dataset
- Commercial use requires minimum 10% token ownership`}
                value={tokenizationProtocol}
                onChange={e => setTokenizationProtocol(e.target.value)}
              />
              <label className="label">
                <span className="label-text-alt">Supports Markdown format / 支持 Markdown 格式</span>
              </label>
