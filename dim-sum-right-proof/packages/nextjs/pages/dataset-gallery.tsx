  ...
  useEffect(() => {
    ...
    const fetchedDatasets = datasetResults
      .map((data, index) => {
        ...
        const [name, uri, , link, bodhi_id] = (data as any).result; // Skip contentHash and licenseId
    ...
    // Fetch abstracts for datasets with bodhi_id (only if it's a numeric bodhi_id)
    fetchedDatasets.forEach(dataset => {
      const bodhiIdNum = parseInt(dataset.bodhi_id);
      if (!isNaN(bodhiIdNum) && bodhiIdNum > 0) {
        fetchAbstract(bodhiIdNum, dataset.id);
      }
    });
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="mb-2">
                  ...
                  <a
                    href={/^\d+$/.test(dataset.link) ? `https://bodhi.wtf/${dataset.link}` : dataset.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="ml-2 text-sm text-blue-600 hover:text-blue-800 truncate block"
                  >
                    {/^\d+$/.test(dataset.link) ? `https://bodhi.wtf/${dataset.link}` : dataset.link}
                  </a>
                </div>

                {/* Bodhi ID or Unique ID */}
                <div className="mb-4">
                  <span className="text-sm text-gray-600">
                    {/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(dataset.bodhi_id)
                      ? "DimSum Unique ID:"
                      : "Bodhi ID:"}
                  </span>
                  {/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(dataset.bodhi_id) ? (
                    <span className="ml-2 text-sm font-mono text-gray-700">{dataset.bodhi_id}</span>
                  ) : (
                    <a
