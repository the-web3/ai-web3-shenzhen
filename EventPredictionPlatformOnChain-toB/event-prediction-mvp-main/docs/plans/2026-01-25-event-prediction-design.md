# Event Prediction 前端设计文档

**版本**: 1.0.0
**日期**: 2026-01-25
**状态**: Draft

---

## 1. 项目概述

### 1.1 产品定位
一个 **Vendor（Dapp）隔离** 的预测事件交易平台：用户必须通过邀请码加入某个 Dapp 后，才能在该 Dapp 的作用域内完成充值、交易、持仓查看与结算。

### 1.2 技术选型

| 层级 | 技术栈 |
|------|--------|
| 前端框架 | Next.js 15 + React 19 + TypeScript |
| 样式 | Tailwind CSS + DaisyUI |
| 状态管理 | Zustand (客户端) + React Query (服务端) |
| Web3 | Wagmi + Viem + RainbowKit |
| 数据库 | Supabase (PostgreSQL) |
| 合约 | Event-Prediction (Foundry, 手动复制 ABI) |
| 链 | Local Hardhat (开发阶段) |

### 1.3 核心设计决策

| 决策点 | 方案 |
|--------|------|
| 用户认证 | 钱包 + 签名验证 → JWT Session |
| 数据同步 | 链上为主，DB 缓存 |
| 邀请码存储 | 明文存储（Hackathon 简化） |
| 交易界面 | 简化市价风格（只输数量） |
| 数据更新 | 轮询（5-10秒） |
| UI 组件 | DaisyUI |

---

## 2. 系统架构

### 2.1 整体架构图

```
┌─────────────────────────────────────────────────────────────┐
│                        Frontend (Next.js 15)                 │
├─────────────────────────────────────────────────────────────┤
│  Pages                          │  State Management          │
│  ├── /join?code=xxx            │  ├── Zustand (全局状态)     │
│  ├── /home                     │  │   ├── activeVendor       │
│  ├── /event/[id]               │  │   ├── userSession        │
│  ├── /portfolio                │  │   └── walletState        │
│  ├── /dapp                     │  └── React Query (服务端)   │
│  └── /admin                    │                             │
├─────────────────────────────────────────────────────────────┤
│  API Routes (/api)              │  Contract Hooks             │
│  ├── /auth/*                   │  ├── useEventPod            │
│  ├── /vendors/*                │  ├── useOrderBookPod        │
│  ├── /events/*                 │  ├── useFundingPod          │
│  ├── /orders/*                 │  └── useFeeVaultPod         │
│  └── /invite/*                 │                             │
├─────────────────────────────────────────────────────────────┤
│                     Supabase (PostgreSQL)                    │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                  Smart Contracts (Local Hardhat)             │
│  PodFactory → EventPod / OrderBookPod / FundingPod / FeeVault│
└─────────────────────────────────────────────────────────────┘
```

### 2.2 数据流原则

1. **写操作**: Frontend → 合约交易 → 成功后 → 同步到 Supabase
2. **读操作**: Frontend → Supabase (快速) → 必要时链上验证
3. **实时性**: 轮询 + 用户操作后手动刷新

---

## 3. 数据库设计 (Supabase)

### 3.1 核心表结构

```sql
-- ============================================================
-- 1. vendors (Vendor 主表，链上同步)
-- ============================================================
CREATE TABLE vendors (
  id BIGSERIAL PRIMARY KEY,
  vendor_id BIGINT NOT NULL UNIQUE,
  vendor_name VARCHAR(128) NOT NULL,
  vendor_address VARCHAR(42) NOT NULL UNIQUE,
  fee_recipient VARCHAR(42) NOT NULL,
  is_active BOOLEAN DEFAULT true,

  -- Pod 地址
  event_pod VARCHAR(42) NOT NULL,
  orderbook_pod VARCHAR(42) NOT NULL,
  funding_pod VARCHAR(42) NOT NULL,
  feevault_pod VARCHAR(42) NOT NULL,

  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_vendors_address ON vendors(vendor_address);
CREATE INDEX idx_vendors_active ON vendors(is_active);

-- ============================================================
-- 2. vendor_invite_codes (邀请码表，纯 DB)
-- ============================================================
CREATE TABLE vendor_invite_codes (
  id BIGSERIAL PRIMARY KEY,
  vendor_id BIGINT NOT NULL REFERENCES vendors(vendor_id),

  code VARCHAR(32) NOT NULL UNIQUE,
  status SMALLINT NOT NULL DEFAULT 1,  -- 1=Active, 0=Revoked, 2=Expired
  max_uses INT NOT NULL DEFAULT 0,     -- 0=无限
  used_count INT NOT NULL DEFAULT 0,
  expires_at TIMESTAMPTZ,

  created_by VARCHAR(42) NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  revoked_at TIMESTAMPTZ
);

CREATE INDEX idx_invite_codes_vendor ON vendor_invite_codes(vendor_id);
CREATE INDEX idx_invite_codes_code ON vendor_invite_codes(code);
CREATE INDEX idx_invite_codes_status ON vendor_invite_codes(status);

-- ============================================================
-- 3. user_vendors (用户-Vendor 关系表，纯 DB)
-- ============================================================
CREATE TABLE user_vendors (
  id BIGSERIAL PRIMARY KEY,
  user_address VARCHAR(42) NOT NULL,
  vendor_id BIGINT NOT NULL REFERENCES vendors(vendor_id),

  joined_at TIMESTAMPTZ DEFAULT NOW(),
  joined_via_invite_id BIGINT REFERENCES vendor_invite_codes(id),
  status SMALLINT NOT NULL DEFAULT 1,  -- 1=Active, 0=Disabled

  UNIQUE(user_address, vendor_id)
);

CREATE INDEX idx_user_vendors_user ON user_vendors(user_address);
CREATE INDEX idx_user_vendors_vendor ON user_vendors(vendor_id);

-- ============================================================
-- 4. events (事件表，链上同步)
-- ============================================================
CREATE TABLE events (
  id BIGSERIAL PRIMARY KEY,
  vendor_id BIGINT NOT NULL REFERENCES vendors(vendor_id),
  event_id BIGINT NOT NULL,

  title VARCHAR(255) NOT NULL,
  description TEXT,
  deadline TIMESTAMPTZ NOT NULL,
  settlement_time TIMESTAMPTZ NOT NULL,

  status SMALLINT NOT NULL,  -- 0=Created, 1=Active, 2=Settled, 3=Cancelled
  creator_address VARCHAR(42) NOT NULL,

  winning_outcome_index SMALLINT,
  outcome_count SMALLINT NOT NULL,
  outcomes JSONB NOT NULL,  -- [{index, name, description}]

  prize_pool NUMERIC(36, 18) DEFAULT 0,
  volume NUMERIC(36, 18) DEFAULT 0,

  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  settled_at TIMESTAMPTZ,

  UNIQUE(vendor_id, event_id)
);

CREATE INDEX idx_events_vendor_status ON events(vendor_id, status);
CREATE INDEX idx_events_vendor_deadline ON events(vendor_id, deadline);

-- ============================================================
-- 5. orders (订单表，链上同步)
-- ============================================================
CREATE TABLE orders (
  id BIGSERIAL PRIMARY KEY,
  vendor_id BIGINT NOT NULL REFERENCES vendors(vendor_id),
  order_id BIGINT NOT NULL,
  user_address VARCHAR(42) NOT NULL,

  event_id BIGINT NOT NULL,
  outcome_index SMALLINT NOT NULL,
  side SMALLINT NOT NULL,  -- 0=Buy, 1=Sell

  price INT NOT NULL,  -- 1-10000 basis points
  amount NUMERIC(36, 18) NOT NULL,
  filled_amount NUMERIC(36, 18) DEFAULT 0,
  remaining_amount NUMERIC(36, 18) NOT NULL,

  status SMALLINT NOT NULL,  -- 0=Pending, 1=Partial, 2=Filled, 3=Cancelled
  token_address VARCHAR(42) NOT NULL,

  tx_hash VARCHAR(66),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),

  UNIQUE(vendor_id, order_id)
);

CREATE INDEX idx_orders_vendor_user ON orders(vendor_id, user_address);
CREATE INDEX idx_orders_vendor_event ON orders(vendor_id, event_id);
CREATE INDEX idx_orders_status ON orders(status);

-- ============================================================
-- 6. user_balances (用户余额，链上同步)
-- ============================================================
CREATE TABLE user_balances (
  id BIGSERIAL PRIMARY KEY,
  vendor_id BIGINT NOT NULL REFERENCES vendors(vendor_id),
  user_address VARCHAR(42) NOT NULL,
  token_address VARCHAR(42) NOT NULL,

  available_balance NUMERIC(36, 18) DEFAULT 0,
  locked_balance NUMERIC(36, 18) DEFAULT 0,

  updated_at TIMESTAMPTZ DEFAULT NOW(),

  UNIQUE(vendor_id, user_address, token_address)
);

CREATE INDEX idx_balances_vendor_user ON user_balances(vendor_id, user_address);

-- ============================================================
-- 7. positions (持仓，链上同步)
-- ============================================================
CREATE TABLE positions (
  id BIGSERIAL PRIMARY KEY,
  vendor_id BIGINT NOT NULL REFERENCES vendors(vendor_id),
  user_address VARCHAR(42) NOT NULL,
  event_id BIGINT NOT NULL,
  outcome_index SMALLINT NOT NULL,
  token_address VARCHAR(42) NOT NULL,

  amount NUMERIC(36, 18) NOT NULL,
  avg_cost NUMERIC(36, 18),

  updated_at TIMESTAMPTZ DEFAULT NOW(),

  UNIQUE(vendor_id, user_address, event_id, outcome_index, token_address)
);

CREATE INDEX idx_positions_vendor_user ON positions(vendor_id, user_address);
CREATE INDEX idx_positions_vendor_event ON positions(vendor_id, event_id);

-- ============================================================
-- 8. trades (成交记录，链上同步)
-- ============================================================
CREATE TABLE trades (
  id BIGSERIAL PRIMARY KEY,
  vendor_id BIGINT NOT NULL REFERENCES vendors(vendor_id),

  buy_order_id BIGINT NOT NULL,
  sell_order_id BIGINT NOT NULL,
  event_id BIGINT NOT NULL,
  outcome_index SMALLINT NOT NULL,

  buyer_address VARCHAR(42) NOT NULL,
  seller_address VARCHAR(42) NOT NULL,

  price INT NOT NULL,
  amount NUMERIC(36, 18) NOT NULL,
  token_address VARCHAR(42) NOT NULL,

  tx_hash VARCHAR(66),
  traded_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_trades_vendor_event ON trades(vendor_id, event_id);
CREATE INDEX idx_trades_buyer ON trades(vendor_id, buyer_address);
CREATE INDEX idx_trades_seller ON trades(vendor_id, seller_address);

-- ============================================================
-- 9. vendor_applications (Dapp 申请，纯 DB)
-- ============================================================
CREATE TABLE vendor_applications (
  id BIGSERIAL PRIMARY KEY,
  applicant_address VARCHAR(42) NOT NULL,
  vendor_name VARCHAR(128) NOT NULL,
  description TEXT,

  status SMALLINT NOT NULL DEFAULT 0,  -- 0=Pending, 1=Approved, 2=Rejected
  reviewed_by VARCHAR(42),
  reviewed_at TIMESTAMPTZ,
  reject_reason TEXT,

  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_applications_status ON vendor_applications(status);
CREATE INDEX idx_applications_applicant ON vendor_applications(applicant_address);

-- ============================================================
-- 10. admin_users (平台管理员，纯 DB)
-- ============================================================
CREATE TABLE admin_users (
  id BIGSERIAL PRIMARY KEY,
  admin_address VARCHAR(42) NOT NULL UNIQUE,
  role VARCHAR(32) NOT NULL DEFAULT 'admin',  -- admin, super_admin
  created_at TIMESTAMPTZ DEFAULT NOW()
);
```

### 3.2 Row Level Security (RLS) 策略

```sql
-- 启用 RLS
ALTER TABLE vendors ENABLE ROW LEVEL SECURITY;
ALTER TABLE user_vendors ENABLE ROW LEVEL SECURITY;
ALTER TABLE events ENABLE ROW LEVEL SECURITY;
ALTER TABLE orders ENABLE ROW LEVEL SECURITY;
ALTER TABLE user_balances ENABLE ROW LEVEL SECURITY;
ALTER TABLE positions ENABLE ROW LEVEL SECURITY;

-- 示例策略：用户只能查看已加入 vendor 的数据
CREATE POLICY "Users can view joined vendors" ON vendors
  FOR SELECT USING (
    vendor_id IN (
      SELECT vendor_id FROM user_vendors
      WHERE user_address = current_setting('app.user_address', true)
    )
  );
```

---

## 4. 认证系统

### 4.1 认证流程

```
┌──────────────────────────────────────────────────────────────┐
│                      用户认证流程                              │
└──────────────────────────────────────────────────────────────┘

1. 连接钱包
   User → RainbowKit Connect → 获取 address

2. 请求签名
   Frontend → 生成 message:
   "Welcome to Event Prediction!\n\nSign this message to verify your wallet.\n\nNonce: {random}\nTimestamp: {iso_time}"

3. 验证签名
   POST /api/auth/verify
   Request: { address, signature, message }
   Backend:
   - verifyMessage(address, message, signature)
   - 生成 JWT: { address, exp: 24h }
   Response: { token, expiresAt }

4. 建立会话
   - 存储 token 到 localStorage
   - 后续请求携带 Authorization: Bearer {token}

5. 检查 Vendor 关系
   GET /api/user/vendors
   - 无记录 → 重定向 /join
   - 有记录 → 设置 activeVendor
```

### 4.2 JWT Payload 结构

```typescript
interface JWTPayload {
  address: string;      // 用户钱包地址
  iat: number;          // 签发时间
  exp: number;          // 过期时间 (24h)
}
```

### 4.3 Zustand Auth Store

```typescript
// stores/useAuthStore.ts
interface AuthState {
  // 状态
  address: string | null;
  token: string | null;
  isAuthenticated: boolean;
  activeVendorId: number | null;
  joinedVendors: Vendor[];

  // Actions
  login: (address: string, token: string) => void;
  logout: () => void;
  setActiveVendor: (vendorId: number) => void;
  setJoinedVendors: (vendors: Vendor[]) => void;

  // Computed
  isVendorOwner: (vendorId: number) => boolean;
  isAdmin: () => boolean;
}
```

---

## 5. 页面设计

### 5.1 路由结构

```
/                        → 重定向逻辑
/join                    → 邀请码加入页（公开）
/home                    → 事件列表（需登录+已加入Vendor）
/event/[eventId]         → 事件详情+交易
/portfolio               → 个人中心
  /portfolio/assets      → 资产页
  /portfolio/history     → 交易历史
/dapp                    → Vendor 管理端
  /dapp/events           → 事件管理
  /dapp/invites          → 邀请码管理
  /dapp/fees             → 费用池
/admin                   → 平台管理端
  /admin/applications    → 申请审核
```

### 5.2 页面详细设计

#### 5.2.1 `/join` - 邀请码加入页

```
┌─────────────────────────────────────────────────────────────┐
│                    Event Prediction                          │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │                 Join a Dapp                          │   │
│  │                                                     │   │
│  │  ┌─────────────────────────────────────────────┐   │   │
│  │  │ Vendor Logo        Vendor Name              │   │   │
│  │  │ [img]              Welcome to MyDapp!       │   │   │
│  │  │                    A prediction market...   │   │   │
│  │  └─────────────────────────────────────────────┘   │   │
│  │                                                     │   │
│  │  Invite Code: [ABC123    ] (从 URL 自动填入)        │   │
│  │                                                     │   │
│  │  Status: ✅ Valid                                   │   │
│  │                                                     │   │
│  │  ┌─────────────────────────────────────────────┐   │   │
│  │  │ 🦊 Connect Wallet                           │   │   │
│  │  └─────────────────────────────────────────────┘   │   │
│  │                                                     │   │
│  │  ┌─────────────────────────────────────────────┐   │   │
│  │  │ 🚀 Join This Dapp                           │   │   │
│  │  └─────────────────────────────────────────────┘   │   │
│  │                                                     │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘

状态处理:
- code 无效: "邀请码无效"
- code 已撤销: "邀请码已失效"
- code 过期: "邀请码已过期"
- code 用尽: "邀请码已达使用上限"
- 已加入: "你已加入该 Dapp" + [进入首页] 按钮
```

#### 5.2.2 `/home` - 事件列表

```
┌─────────────────────────────────────────────────────────────┐
│  [Logo]  [Vendor: MyDapp ▼]           [Portfolio] [0x12..ab]│
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Welcome to MyDapp                                          │
│  Your balance: 1,000.00 USDT                               │
│                                                             │
│  ┌─ Filter ─────────────────────────────────────────────┐  │
│  │ [All] [Active] [Pending] [Settled]     🔍 Search     │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌─ Event Card ─────────────────────────────────────────┐  │
│  │ 🟢 Active                           Deadline: 5 days │  │
│  │                                                       │  │
│  │ 2024 美国大选获胜者                                     │  │
│  │ Who will win the 2024 US Presidential Election?      │  │
│  │                                                       │  │
│  │ ┌──────────────┐  ┌──────────────┐                   │  │
│  │ │ Candidate A  │  │ Candidate B  │                   │  │
│  │ │ 60%          │  │ 40%          │                   │  │
│  │ └──────────────┘  └──────────────┘                   │  │
│  │                                                       │  │
│  │ Prize Pool: 10,000 USDT    Volume: 25,000 USDT       │  │
│  │                                         [Trade →]    │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌─ Event Card ─────────────────────────────────────────┐  │
│  │ 🟡 Pending                          Settlement: 2 days│  │
│  │ ...                                                   │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 5.2.3 `/event/[id]` - 事件详情 + 交易

```
┌─────────────────────────────────────────────────────────────┐
│  [← Back]  [Vendor: MyDapp]               [Portfolio] [0x..]│
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌─ Event Info ─────────────────────────────────────────┐  │
│  │ 2024 美国大选获胜者                          🟢 Active │  │
│  │                                                       │  │
│  │ Who will win the 2024 US Presidential Election?      │  │
│  │                                                       │  │
│  │ Deadline: 2024-11-05 12:00 UTC (5 days left)         │  │
│  │ Settlement: 2024-11-06 00:00 UTC                     │  │
│  │                                                       │  │
│  │ Prize Pool: 10,000 USDT    Volume: 25,000 USDT       │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌─ Outcomes ───────────────┐  ┌─ Trade Panel ──────────┐  │
│  │                          │  │                        │  │
│  │ ┌──────────────────────┐ │  │ Buy: Candidate A       │  │
│  │ │ ○ Candidate A        │ │  │                        │  │
│  │ │   Price: 0.60 (60%)  │ │  │ Your Balance: 1,000 USDT│ │
│  │ │   Your Pos: 100 Long │ │  │ Your Position: 100 Long│  │
│  │ └──────────────────────┘ │  │                        │  │
│  │                          │  │ ┌────────────────────┐ │  │
│  │ ┌──────────────────────┐ │  │ │ [Buy]  [Sell]      │ │  │
│  │ │ ○ Candidate B        │ │  │ └────────────────────┘ │  │
│  │ │   Price: 0.40 (40%)  │ │  │                        │  │
│  │ │   Your Pos: 0        │ │  │ Amount (USDT):         │  │
│  │ └──────────────────────┘ │  │ [_______________] MAX  │  │
│  │                          │  │                        │  │
│  └──────────────────────────┘  │ Est. Price: 0.60       │  │
│                                │ Est. Fee: 0.30 USDT    │  │
│                                │ Est. Shares: 166.67    │  │
│                                │                        │  │
│                                │ [Confirm Buy]          │  │
│                                └────────────────────────┘  │
│                                                             │
│  ┌─ My Orders ──────────────────────────────────────────┐  │
│  │                                           [Refresh]  │  │
│  │ Side │ Outcome │ Amount │ Filled │ Status  │ Action  │  │
│  │ ─────┼─────────┼────────┼────────┼─────────┼──────── │  │
│  │ Buy  │ A       │ 100    │ 60     │ Partial │ [Cancel]│  │
│  │ Buy  │ A       │ 50     │ 50     │ Filled  │ -       │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘

Cancelled 事件展示:
┌─────────────────────────────────────────────────────────────┐
│  ⚠️ 警告: 该事件已被取消                                      │
│  取消的事件不会退款、不会赔偿、不会结算。                        │
│  你在该事件中的持仓将无法兑换。                                 │
└─────────────────────────────────────────────────────────────┘
```

#### 5.2.4 `/portfolio` - 个人中心

```
┌─────────────────────────────────────────────────────────────┐
│  [Logo]  [Vendor: MyDapp ▼]               [Portfolio] [0x..]│
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  My Portfolio                                               │
│  ─────────────────────────────────────────────────────────  │
│  [Assets]  [Positions]  [History]                          │
│                                                             │
│  ┌─ Assets Tab ─────────────────────────────────────────┐  │
│  │                                                       │  │
│  │  Available Balance          Locked Balance            │  │
│  │  1,000.00 USDT             250.00 USDT               │  │
│  │                                                       │  │
│  │  [Deposit]  [Withdraw]                               │  │
│  │                                                       │  │
│  │  Deposit Modal:                                       │  │
│  │  ┌─────────────────────────────────────────────┐     │  │
│  │  │ Amount: [________] USDT                     │     │  │
│  │  │ Wallet Balance: 5,000 USDT                  │     │  │
│  │  │ [Approve] → [Deposit]                       │     │  │
│  │  └─────────────────────────────────────────────┘     │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌─ Positions Tab ──────────────────────────────────────┐  │
│  │                                                       │  │
│  │  Event           │ Outcome │ Amount │ Avg Cost │ PnL  │  │
│  │  ─────────────────┼─────────┼────────┼──────────┼───── │  │
│  │  2024 大选        │ A       │ 100    │ 0.55     │ +5%  │  │
│  │  BTC 价格预测     │ >100k   │ 50     │ 0.30     │ +20% │  │
│  │                                                       │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌─ History Tab ────────────────────────────────────────┐  │
│  │                                                       │  │
│  │  Time       │ Type  │ Event │ Amount │ Price │ TxHash│  │
│  │  ───────────┼───────┼───────┼────────┼───────┼────── │  │
│  │  2024-01-20 │ Buy   │ A     │ 50     │ 0.60  │ 0x... │  │
│  │  2024-01-19 │ Sell  │ B     │ 30     │ 0.42  │ 0x... │  │
│  │                                                       │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 5.2.5 `/dapp` - Vendor 管理端

```
┌─────────────────────────────────────────────────────────────┐
│  [Logo]  [← Back to Home]                    [Vendor: MyDapp]│
├────────────────┬────────────────────────────────────────────┤
│  Sidebar       │  Main Content                              │
│                │                                            │
│  ┌──────────┐  │  ┌─ Events Tab ────────────────────────┐  │
│  │ 📋 Events │  │  │                                     │  │
│  │ 🎟️ Invites│  │  │ [+ Create Event]                    │  │
│  │ 💰 Fees   │  │  │                                     │  │
│  │ ⚙️ Settings│  │  │ ┌─────────────────────────────────┐ │  │
│  └──────────┘  │  │ │ 2024 大选               🟢 Active │ │  │
│                │  │ │ Deadline: 2024-11-05              │ │  │
│                │  │ │ Pool: 10,000  Volume: 25,000      │ │  │
│                │  │ │              [View] [Cancel]      │ │  │
│                │  │ └─────────────────────────────────┘ │  │
│                │  │                                     │  │
│                │  └─────────────────────────────────────┘  │
│                │                                            │
│                │  Create Event Modal:                       │
│                │  ┌─────────────────────────────────────┐  │
│                │  │ Title: [____________________]       │  │
│                │  │ Description: [______________]      │  │
│                │  │ Deadline: [____] Settlement: [____]│  │
│                │  │                                     │  │
│                │  │ Outcomes:                           │  │
│                │  │ 1. [Option A    ] [+ Add]           │  │
│                │  │ 2. [Option B    ] [Remove]          │  │
│                │  │                                     │  │
│                │  │ [Create Event]                      │  │
│                │  └─────────────────────────────────────┘  │
├────────────────┼────────────────────────────────────────────┤
│                │  ┌─ Invites Tab ───────────────────────┐  │
│                │  │                                     │  │
│                │  │ [+ Generate Invite Code]            │  │
│                │  │                                     │  │
│                │  │ Code   │ Status │ Used │ Expires │ Act│  │
│                │  │ ABC123 │ Active │ 5/∞  │ Never   │[R] │  │
│                │  │ XYZ789 │ Revoked│ 3/10 │ -       │ -  │  │
│                │  │                                     │  │
│                │  │ Generate Modal:                     │  │
│                │  │ Max Uses: [0] (0=unlimited)         │  │
│                │  │ Expires: [Optional date]            │  │
│                │  │ [Generate]                          │  │
│                │  │                                     │  │
│                │  │ ✅ Generated: ABC123                │  │
│                │  │ Link: https://app.com/join?code=ABC123│ │
│                │  │ [Copy Link]                         │  │
│                │  └─────────────────────────────────────┘  │
├────────────────┼────────────────────────────────────────────┤
│                │  ┌─ Fees Tab ──────────────────────────┐  │
│                │  │                                     │  │
│                │  │ Pending Fees:  150.00 USDT          │  │
│                │  │ [Push to Available]                 │  │
│                │  │                                     │  │
│                │  │ Available:     500.00 USDT          │  │
│                │  │ [Withdraw]                          │  │
│                │  │                                     │  │
│                │  │ Withdrawal History:                 │  │
│                │  │ 2024-01-15 │ 200 USDT │ 0x1234...   │  │
│                │  │ 2024-01-10 │ 100 USDT │ 0x5678...   │  │
│                │  │                                     │  │
│                │  └─────────────────────────────────────┘  │
└────────────────┴────────────────────────────────────────────┘
```

#### 5.2.6 `/admin` - 平台管理端

```
┌─────────────────────────────────────────────────────────────┐
│  [Logo]  Admin Panel                              [0x12..ab]│
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Vendor Applications                                        │
│  ─────────────────────────────────────────────────────────  │
│  [Pending: 3]  [Approved]  [Rejected]                      │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Application #12                         🟡 Pending   │   │
│  │                                                     │   │
│  │ Applicant: 0x1234...abcd                           │   │
│  │ Vendor Name: CryptoPredict                         │   │
│  │ Description: A prediction market for crypto...     │   │
│  │ Applied: 2024-01-20 14:30                          │   │
│  │                                                     │   │
│  │ [Approve] [Reject]                                 │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  Reject Modal:                                              │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Reason for rejection:                               │   │
│  │ [________________________________]                  │   │
│  │ [Confirm Reject]                                    │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## 6. API 设计

### 6.1 API 路由结构

```
/api
├── /auth
│   ├── POST /verify          # 验证签名，返回 JWT
│   └── GET  /me              # 获取当前用户信息
│
├── /vendors
│   ├── GET  /                # 获取用户已加入的 vendors
│   ├── GET  /:id             # 获取 vendor 详情
│   └── POST /sync            # 从链上同步 vendor 数据
│
├── /invite
│   ├── GET  /validate        # 验证邀请码 ?code=xxx
│   ├── POST /join            # 使用邀请码加入
│   ├── POST /generate        # [Vendor] 生成邀请码
│   └── POST /revoke          # [Vendor] 撤销邀请码
│
├── /events
│   ├── GET  /                # 获取事件列表 ?vendor_id=&status=
│   ├── GET  /:id             # 获取事件详情
│   ├── POST /                # [Vendor] 创建事件
│   ├── POST /:id/sync        # 从链上同步事件数据
│   └── POST /:id/cancel      # [Vendor] 取消事件
│
├── /orders
│   ├── GET  /                # 获取用户订单 ?vendor_id=&event_id=
│   ├── GET  /:id             # 获取订单详情
│   └── POST /sync            # 从链上同步订单数据
│
├── /balances
│   ├── GET  /                # 获取用户余额 ?vendor_id=
│   └── POST /sync            # 从链上同步余额
│
├── /positions
│   ├── GET  /                # 获取用户持仓 ?vendor_id=&event_id=
│   └── POST /sync            # 从链上同步持仓
│
├── /applications            # [Admin]
│   ├── GET  /                # 获取申请列表 ?status=
│   ├── POST /                # 提交申请
│   ├── POST /:id/approve     # 批准申请
│   └── POST /:id/reject      # 拒绝申请
│
└── /admin
    └── GET  /check           # 检查是否为管理员
```

### 6.2 核心 API 详细设计

#### POST /api/auth/verify

```typescript
// Request
{
  address: string;      // 钱包地址
  message: string;      // 签名的消息
  signature: string;    // 签名结果
}

// Response 200
{
  token: string;        // JWT token
  expiresAt: number;    // 过期时间戳
}

// Response 401
{
  error: "INVALID_SIGNATURE" | "SIGNATURE_EXPIRED"
}
```

#### GET /api/invite/validate

```typescript
// Request
GET /api/invite/validate?code=ABC123

// Response 200
{
  valid: true;
  vendor: {
    vendor_id: number;
    vendor_name: string;
    // logo, description if available
  };
  alreadyJoined: boolean;  // 是否已加入
}

// Response 400
{
  valid: false;
  error: "INVALID_CODE" | "CODE_REVOKED" | "CODE_EXPIRED" | "CODE_EXHAUSTED"
}
```

#### POST /api/invite/join

```typescript
// Request
{
  code: string;
}

// Response 200
{
  success: true;
  vendor: Vendor;
}

// Response 400
{
  error: "INVALID_CODE" | "ALREADY_JOINED" | ...
}
```

#### POST /api/events (Vendor Only)

```typescript
// Request
{
  vendor_id: number;
  title: string;
  description: string;
  deadline: string;        // ISO timestamp
  settlement_time: string;
  outcomes: {
    name: string;
    description?: string;
  }[];
  tx_hash: string;         // 链上创建事件的交易 hash
  event_id: number;        // 链上返回的 event_id
}

// Response 201
{
  id: number;
  event_id: number;
  // ... full event object
}
```

---

## 7. 合约集成

### 7.1 ABI 文件结构

```
packages/nextjs/
├── contracts/
│   ├── abis/
│   │   ├── PodFactory.json
│   │   ├── EventPod.json
│   │   ├── OrderBookPod.json
│   │   ├── FundingPod.json
│   │   └── FeeVaultPod.json
│   ├── addresses.ts       # 合约地址配置
│   └── index.ts           # 导出
```

### 7.2 合约地址配置

```typescript
// contracts/addresses.ts
export const CONTRACT_ADDRESSES = {
  // Local Hardhat
  31337: {
    podFactory: "0x...",
    // 其他全局合约
  },
  // Sepolia (未来)
  11155111: {
    podFactory: "0x...",
  }
} as const;
```

### 7.3 Wagmi Hooks 封装

```typescript
// hooks/contracts/usePodFactory.ts
export function usePodFactory() {
  const { data: vendorInfo } = useReadContract({
    address: CONTRACT_ADDRESSES[chainId].podFactory,
    abi: PodFactoryAbi,
    functionName: 'getVendorInfo',
    args: [vendorId],
  });

  return { vendorInfo };
}

// hooks/contracts/useEventPod.ts
export function useEventPod(eventPodAddress: Address) {
  // Read: getEvent, getEventStatus
  const { data: event } = useReadContract({
    address: eventPodAddress,
    abi: EventPodAbi,
    functionName: 'getEvent',
    args: [eventId],
  });

  // Write: createEvent
  const { writeContract: createEvent } = useWriteContract();

  return { event, createEvent };
}

// hooks/contracts/useOrderBookPod.ts
export function useOrderBookPod(orderBookPodAddress: Address) {
  // Read: getBestBid, getBestAsk, getOrder, getPosition
  // Write: placeOrder, cancelOrder
}

// hooks/contracts/useFundingPod.ts
export function useFundingPod(fundingPodAddress: Address) {
  // Read: getUserBalance, getLongPosition
  // Write: deposit, withdraw, mintCompleteSet, burnCompleteSet
}

// hooks/contracts/useFeeVaultPod.ts
export function useFeeVaultPod(feeVaultPodAddress: Address) {
  // Read: getFeeBalance, calculateFee
  // Write: withdrawFee
}
```

### 7.4 交易流程示例

```typescript
// 下单流程
async function handlePlaceOrder() {
  // 1. 调用合约
  const tx = await placeOrder({
    address: orderBookPodAddress,
    abi: OrderBookPodAbi,
    functionName: 'placeOrder',
    args: [eventId, outcomeIndex, side, price, amount, tokenAddress],
  });

  // 2. 等待交易确认
  const receipt = await waitForTransaction({ hash: tx });

  // 3. 解析事件获取 orderId
  const orderId = parseOrderPlacedEvent(receipt.logs);

  // 4. 同步到数据库
  await fetch('/api/orders/sync', {
    method: 'POST',
    body: JSON.stringify({
      vendor_id: activeVendorId,
      order_id: orderId,
      tx_hash: tx,
    }),
  });

  // 5. 刷新 UI
  queryClient.invalidateQueries(['orders']);
}
```

---

## 8. 项目文件结构

```
packages/nextjs/
├── app/
│   ├── layout.tsx                 # Root layout
│   ├── page.tsx                   # / → 重定向逻辑
│   ├── join/
│   │   └── page.tsx               # /join
│   ├── home/
│   │   └── page.tsx               # /home
│   ├── event/
│   │   └── [eventId]/
│   │       └── page.tsx           # /event/[eventId]
│   ├── portfolio/
│   │   ├── page.tsx               # /portfolio (默认 assets)
│   │   └── layout.tsx             # Portfolio layout with tabs
│   ├── dapp/
│   │   ├── page.tsx               # /dapp (默认 events)
│   │   └── layout.tsx             # Dapp layout with sidebar
│   ├── admin/
│   │   └── page.tsx               # /admin
│   └── api/
│       ├── auth/
│       │   ├── verify/route.ts
│       │   └── me/route.ts
│       ├── vendors/
│       │   └── route.ts
│       ├── invite/
│       │   ├── validate/route.ts
│       │   ├── join/route.ts
│       │   ├── generate/route.ts
│       │   └── revoke/route.ts
│       ├── events/
│       │   └── route.ts
│       ├── orders/
│       │   └── route.ts
│       ├── balances/
│       │   └── route.ts
│       ├── positions/
│       │   └── route.ts
│       └── applications/
│           └── route.ts
│
├── components/
│   ├── layout/
│   │   ├── Header.tsx
│   │   ├── Footer.tsx
│   │   ├── VendorSwitcher.tsx
│   │   └── Sidebar.tsx
│   ├── auth/
│   │   ├── ConnectButton.tsx
│   │   ├── SignInButton.tsx
│   │   └── AuthGuard.tsx
│   ├── events/
│   │   ├── EventCard.tsx
│   │   ├── EventList.tsx
│   │   ├── EventDetail.tsx
│   │   └── OutcomeCard.tsx
│   ├── trading/
│   │   ├── TradePanel.tsx
│   │   ├── OrderList.tsx
│   │   └── PositionList.tsx
│   ├── portfolio/
│   │   ├── BalanceCard.tsx
│   │   ├── DepositModal.tsx
│   │   └── WithdrawModal.tsx
│   ├── dapp/
│   │   ├── EventManager.tsx
│   │   ├── CreateEventModal.tsx
│   │   ├── InviteManager.tsx
│   │   └── FeeManager.tsx
│   └── admin/
│       └── ApplicationList.tsx
│
├── hooks/
│   ├── contracts/
│   │   ├── usePodFactory.ts
│   │   ├── useEventPod.ts
│   │   ├── useOrderBookPod.ts
│   │   ├── useFundingPod.ts
│   │   └── useFeeVaultPod.ts
│   ├── useAuth.ts
│   ├── useVendor.ts
│   └── usePolling.ts
│
├── stores/
│   ├── useAuthStore.ts
│   └── useVendorStore.ts
│
├── lib/
│   ├── supabase/
│   │   ├── client.ts              # Browser client
│   │   ├── server.ts              # Server client
│   │   └── types.ts               # Database types
│   ├── auth/
│   │   ├── jwt.ts
│   │   └── verify.ts
│   └── utils/
│       ├── format.ts
│       └── constants.ts
│
├── contracts/
│   ├── abis/
│   │   ├── PodFactory.json
│   │   ├── EventPod.json
│   │   ├── OrderBookPod.json
│   │   ├── FundingPod.json
│   │   └── FeeVaultPod.json
│   ├── addresses.ts
│   └── index.ts
│
└── types/
    ├── vendor.ts
    ├── event.ts
    ├── order.ts
    └── user.ts
```

---

## 9. 环境配置

### 9.1 环境变量

```bash
# .env.local

# Supabase
NEXT_PUBLIC_SUPABASE_URL=https://xxx.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
SUPABASE_SERVICE_ROLE_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...

# JWT Secret (for custom auth)
JWT_SECRET=your-super-secret-key-at-least-32-chars

# Web3
NEXT_PUBLIC_ALCHEMY_API_KEY=xxx
NEXT_PUBLIC_WALLET_CONNECT_PROJECT_ID=xxx

# Chain Config
NEXT_PUBLIC_TARGET_CHAIN_ID=31337  # Local Hardhat

# Contract Addresses (Local)
NEXT_PUBLIC_POD_FACTORY_ADDRESS=0x...
```

### 9.2 Supabase 客户端配置

```typescript
// lib/supabase/client.ts
import { createBrowserClient } from '@supabase/ssr';

export function createClient() {
  return createBrowserClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
  );
}

// lib/supabase/server.ts
import { createServerClient } from '@supabase/ssr';
import { cookies } from 'next/headers';

export async function createServerSupabaseClient() {
  const cookieStore = await cookies();

  return createServerClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        getAll() {
          return cookieStore.getAll();
        },
        setAll(cookiesToSet) {
          cookiesToSet.forEach(({ name, value, options }) => {
            cookieStore.set(name, value, options);
          });
        },
      },
    }
  );
}
```

---

## 10. 实现阶段划分

### Phase 1: 基础设施 (Day 1)

- [ ] 配置 Supabase 连接
- [ ] 创建数据库表（运行 SQL）
- [ ] 设置环境变量
- [ ] 复制合约 ABI
- [ ] 实现认证系统 (/api/auth/*)
- [ ] 创建 Zustand stores
- [ ] 实现 AuthGuard 组件

### Phase 2: 邀请码与加入流程 (Day 1-2)

- [ ] /join 页面
- [ ] /api/invite/* API
- [ ] VendorSwitcher 组件
- [ ] 加入成功后的重定向逻辑

### Phase 3: 事件列表与详情 (Day 2)

- [ ] /home 页面
- [ ] EventCard, EventList 组件
- [ ] /event/[id] 页面
- [ ] EventDetail, OutcomeCard 组件
- [ ] /api/events/* API

### Phase 4: 交易功能 (Day 2-3)

- [ ] TradePanel 组件
- [ ] 合约 hooks (useOrderBookPod, useFundingPod)
- [ ] 下单流程（合约调用 + DB 同步）
- [ ] OrderList 组件
- [ ] 撤单功能

### Phase 5: Portfolio (Day 3)

- [ ] /portfolio 页面
- [ ] BalanceCard, DepositModal, WithdrawModal
- [ ] PositionList 组件
- [ ] 交易历史

### Phase 6: Vendor 管理端 (Day 3-4)

- [ ] /dapp 页面布局
- [ ] EventManager + CreateEventModal
- [ ] InviteManager (生成/撤销邀请码)
- [ ] FeeManager (费用池管理)

### Phase 7: Admin 管理端 (Day 4)

- [ ] /admin 页面
- [ ] ApplicationList 组件
- [ ] 审核流程

### Phase 8: 优化与测试 (Day 4-5)

- [ ] 错误处理优化
- [ ] Loading 状态
- [ ] 响应式布局调整
- [ ] E2E 测试关键流程

---

## 11. 验收清单 (Demo Checklist)

参考 PRD Section 8：

1. [ ] 通过邀请码链接进入 `/join`
2. [ ] 连接钱包 → 签名 → 加入 Dapp 成功 → 自动进入 `/home`
3. [ ] 首页看到仅该 vendor 的事件列表
4. [ ] 进入事件页下单，Partial 时能在"我的订单"看到 remaining
5. [ ] 可以撤销 Pending/Partial 订单
6. [ ] Portfolio 显示该 vendor 的余额/持仓/流水
7. [ ] 切换到另一个已加入 vendor：数据完全切换且互不影响
8. [ ] Vendor 端生成/撤销邀请码
9. [ ] Vendor 端创建事件、取消事件
10. [ ] Vendor 端查看/提取费用池
11. [ ] Admin 端审核 Dapp 申请

---

## 附录 A: 常量定义

```typescript
// lib/utils/constants.ts

// 事件状态
export const EVENT_STATUS = {
  CREATED: 0,
  ACTIVE: 1,
  SETTLED: 2,
  CANCELLED: 3,
} as const;

// 订单状态
export const ORDER_STATUS = {
  PENDING: 0,
  PARTIAL: 1,
  FILLED: 2,
  CANCELLED: 3,
} as const;

// 订单方向
export const ORDER_SIDE = {
  BUY: 0,
  SELL: 1,
} as const;

// 邀请码状态
export const INVITE_STATUS = {
  REVOKED: 0,
  ACTIVE: 1,
  EXPIRED: 2,
} as const;

// 申请状态
export const APPLICATION_STATUS = {
  PENDING: 0,
  APPROVED: 1,
  REJECTED: 2,
} as const;

// 价格精度
export const PRICE_PRECISION = 10000;  // 10000 = 100%

// 轮询间隔
export const POLLING_INTERVAL = 10000;  // 10 seconds
```

---

## 附录 B: 类型定义

```typescript
// types/vendor.ts
export interface Vendor {
  id: number;
  vendor_id: number;
  vendor_name: string;
  vendor_address: string;
  fee_recipient: string;
  is_active: boolean;
  event_pod: string;
  orderbook_pod: string;
  funding_pod: string;
  feevault_pod: string;
  created_at: string;
  updated_at: string;
}

// types/event.ts
export interface Outcome {
  index: number;
  name: string;
  description?: string;
}

export interface Event {
  id: number;
  vendor_id: number;
  event_id: number;
  title: string;
  description: string;
  deadline: string;
  settlement_time: string;
  status: number;
  creator_address: string;
  winning_outcome_index: number | null;
  outcome_count: number;
  outcomes: Outcome[];
  prize_pool: string;
  volume: string;
  created_at: string;
  updated_at: string;
  settled_at: string | null;
}

// types/order.ts
export interface Order {
  id: number;
  vendor_id: number;
  order_id: number;
  user_address: string;
  event_id: number;
  outcome_index: number;
  side: number;
  price: number;
  amount: string;
  filled_amount: string;
  remaining_amount: string;
  status: number;
  token_address: string;
  tx_hash: string;
  created_at: string;
  updated_at: string;
}

// types/user.ts
export interface UserBalance {
  id: number;
  vendor_id: number;
  user_address: string;
  token_address: string;
  available_balance: string;
  locked_balance: string;
  updated_at: string;
}

export interface Position {
  id: number;
  vendor_id: number;
  user_address: string;
  event_id: number;
  outcome_index: number;
  token_address: string;
  amount: string;
  avg_cost: string | null;
  updated_at: string;
}
```

---

**文档结束**
